...
Because `users.db` and `faiss_index/` are generated locally, either:
- commit them too (simplest — they're small, static demo data), **or**
- add a one-time setup step in `streamlit_app.py` that calls
  `create_and_seed_database()` and the ingestion pipeline automatically
  on first load if they're missing.

For this assessment, the simplest path is to just run `create_db.py` and
`ingest.py` locally first, then commit the generated `users.db` and
`faiss_index/` folder along with the code.
